...
PVOID PortBender::ProcessPacket(Packet* packet, BOOL redirect) {
	...
	modified_packet_len = (UINT16)sizeof(TCP_PACKET) + packet->payload_len;
	...
	if (packet->payload != NULL) {
		WinDivertHelperCalcChecksums((PVOID)modified_packet, modified_packet_len, &packet->addr, 0);
	}
	else {
		WinDivertHelperCalcChecksums((PVOID)modified_packet, packet->packet_len, &packet->addr, 0);
	}
